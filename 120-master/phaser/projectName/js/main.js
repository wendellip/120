...
MainMenu.prototype = 
{
	...
	preload: function()
	{
		...
		game.load.audio('MainMusic', 'assets/sound/Obliteration'); //music provided by Kevin MacLeod Under Attribution CC license. 
		game.load.audio('wonder/joy', 'assets/sound/Almost New');  //music provided by Kevin MacLeod Under Attribution CC license. 
		game.load.audio('WoodThump', 'assets/sound/WoodThump');
		game.load.audio('Whoosh', 'assets/sound/Whoosh');
		game.load.audio('armtake','assets/sound/Button_Press_5-Marianne_Gagnon-1212299245';//sound recoreded and provided under atribution liscnece by Marianne Gagnon, 2011
	},
	create: function()
	{
...
tutorial2.prototype = 
{
	...
	update: function()
	{
		...
		if(this.door.checkoverlap(this.player.sprite(), this.handstation.sprite()))
		{
			...
			{
				currentsound=game.audio.add('armtake');
				this.hand = this.handstation.takearm(currentsound);
				this.player.addChild(game.add.existing(this.hand));
			}
		}
		else if(game.input.activePointer.justReleased() && this.hand != undefined)
		{
			currentsound=game.audio.add('whoosh');
			currentsound.play();
			var timer = game.time.create(game,true);
			timer.add(1000, currentsound.stop(), this, newhand);
			timer.start();
